...
class MessageMixin(ClientProtocol):
    ...
    async def send_message(
        ...
        try:
            ...
            if data.get("payload") and "message" in data["payload"]:
                msg = Message.from_dict(data["payload"]["message"])
                self.logger.debug("send_message result: %r", msg)
                return msg
            else:
                # Если message нет в payload, создаем фиктивное сообщение
                self.logger.debug("No message in payload, message sent successfully")
                return None
        except Exception:
