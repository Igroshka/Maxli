...
class API:
    ...
    async def await_chat_id(self, message):
        ...
        if hasattr(self, 'me') and self.me and message.sender == self.me.id:
            ...
            # Если диалог не найден, возвращаем None
            print(f"⚠️ Чат 'Избранное' не найден в диалогах, пропускаем сообщение")
            return None
    ...
    async def send(self, chat_id, text, markdown=False, **kwargs):
        ...
        if chat_id == self.me.id:
            ...
            # Проверяем, что чат "Избранное" существует в диалогах
            if not self.client.dialogs or not any(d.id == chat_id for d in self.client.dialogs):
                print(f"⚠️ Чат 'Избранное' не найден в диалогах, пропускаем отправку")
                return None
