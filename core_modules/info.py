...
class InfoModule:
    ...
    async def info_command(self, api, message, args):
        ...
        # Формируем текст инфо (можно кастомизировать через config)
                    ...
                    async with aiohttp.ClientSession() as session:
                        async with session.get(banner_url) as resp:
                            if resp.status != 200:
                                ...
                                await api.send_message(chat_id, info_text, notify=True)
                                return
        ...
        # Используем send_message, так как исходное уже удалено
        chat_id = getattr(message, 'chat_id', None)
        if not chat_id:
            chat_id = await api.await_chat_id(message)
        await api.send_message(chat_id, info_text, notify=True)
